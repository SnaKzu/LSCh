...
if __name__ == "__main__":
    logger.info("=" * 70)
    logger.info("INICIANDO NORMALIZACIÓN DE MUESTRAS")
    logger.info("=" * 70)
    ...
    logger.info(f"Procesando {len(word_ids)} palabras con {MODEL_FRAMES} frames objetivo")
    
    for idx, word_id in enumerate(word_ids, 1):
        word_path = os.path.join(FRAME_ACTIONS_PATH, word_id)
        if os.path.isdir(word_path):
            logger.info(f"[{idx}/{len(word_ids)}] Normalizando '{word_id}'...")
            try:
                process_directory(word_path, MODEL_FRAMES)
                logger.info(f"  ✓ '{word_id}' completado")
            except Exception as e:
                logger.error(f"  ❌ Error en '{word_id}': {e}", exc_info=True)
